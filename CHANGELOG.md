...
### Added
- **Standalone OmniGraph is live.** Repo public at https://github.com/Amitshukla2308/omnigraph (clean history via `git filter-repo`); package published at https://pypi.org/project/informed-vibe-omnigraph/0.0.1/ . `pip install informed-vibe-omnigraph` works against any tool that reads the `og_artifacts/` contract — Atelier vendors a copy at `omnigraph/` for first-run ergonomics, but the standalone makes the substrate-vs-UX boundary real again and enables Cursor / Continue / Cline interop.
