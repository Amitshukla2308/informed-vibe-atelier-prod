...
└── projects/<ProjectName>/
    ...
    │   ├── <kind>.md                          authored (founder or {{agent_name}})
...
<ent-top n="5">project-a:Proj atelier:Proj some-tool:Tool another-project:Proj some-cli:Tool</ent-top>
</user-profile>
```
...
- **Initial bootstrap:** once, `omnigraph pipeline --sessions <ai_conversations_dir> --out <atelier_root>/projects/<P>/brain/personal/` populates everything.
...
omnigraph pipeline \
  ...
  --atelier-root ~/informed-vibe-atelier --user-id <uuid>

# Compile a system-prompt block into Atelier's canonical path.
omnigraph compile light_ir --atelier-root ~/informed-vibe-atelier --user-id <uuid>
omnigraph compile claude_md --atelier-root ~/informed-vibe-atelier --user-id <uuid>
omnigraph compile boot_context --atelier-root ~/informed-vibe-atelier --user-id <uuid>
...
omnigraph migrate --atelier-root ~/informed-vibe-atelier --user-id <uuid>
omnigraph migrate --atelier-root ~/informed-vibe-atelier --user-id <uuid> --dry-run
...
omnigraph canonicalize --atelier-root ~/informed-vibe-atelier --project <ProjectName>
omnigraph canonicalize --atelier-root ~/informed-vibe-atelier --project <ProjectName> --dry-run

# Gap audit of Atelier's project domain_brain/ (scores coverage, flags gaps).
omnigraph domain-brain --project-root <atelier_root>/projects/<ProjectName>
omnigraph domain-brain --project-root <atelier_root>/projects/<ProjectName> --json
```
...
python3 src/omnigraph_cli.py pipeline \
  --sessions pilot/qwen --sessions pilot/full \
  --atelier-root ~/informed-vibe-atelier --user-id "$(cat ~/atelier/data/current_user_uuid 2>/dev/null || echo default)"
python3 src/omnigraph_cli.py compile light_ir \
  --atelier-root ~/informed-vibe-atelier --user-id "$(cat ~/atelier/data/current_user_uuid 2>/dev/null || echo default)"
```

Boot Atelier against any project. Confirm: (a) system prompt contains the `<user-profile>` block, (b) Personal Brain view lists the top entities, (c) no crash when `brain/personal/` is emptied.
...
**Questions / schema requests from Atelier side:** file an issue on the OmniGraph repo.
...
{
  "session_id": "...",
  "atelier_user_id": "founder-slug",  // NEW — stable per human, not per auth
  "project": "<ProjectName>",
...
4. **Cross-user visibility (optional).** If the founder needs to see the *shape* of a co-founder's collaboration patterns (onboarding, team health), the sanitize levels (`named_stripped` / `entities_removed`) already exist in OmniGraph. Atelier wires a "team view" that compiles at `sanitize=named_stripped` — the founder sees the co-founder's mental moves + drift patterns, not their specific project entities or concerns.
...
- Never cross-contaminate: canonicalize_slugs / global_profile / entities are strictly per-user; a target_id named `"project-x"` in one founder's brain and `"project-x"` in another founder's brain are independent records with independent mention histories.
